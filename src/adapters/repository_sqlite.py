...
if __name__=="__main__":
    ...
    u=s.get(user_id=1)
    for t1 in u.tickets:
        print(u.tickets[t1].statuses)
    new_user=User(user_id=0,name="new user",client=Client(client_id=1,name="client",status=ClientStatusEnabled()),status=UserStatusEnabled())
    s.save(user=new_user)
    print(new_user)
    s.delete(user_id=u.user_id)
    connect.commit()
    connect.close()
